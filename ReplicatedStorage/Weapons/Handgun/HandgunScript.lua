...
local MinSpread = 0.005 -- In radians

local MaxSpread = 0.07 -- In radians
...
function RayCast(startPos, vec, rayLength)
	...
	if hitObject and hitPos then
		...
		if RayIgnoreCheck(hitObject, hitPos) and distance > 0 then			return RayCast(hitPos, vec, distance)
		end
	end
	...
	if MyHumanoid and MyHumanoid.Health > 0 then
		...
		while LeftButtonDown and AmmoInClip > 0 and not Reloading do
			...
			if MyMouse then
				...
				local bullet				if hitObject then
